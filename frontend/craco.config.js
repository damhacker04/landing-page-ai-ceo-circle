...
webpackConfig.devServer = (devServerConfig) => {
  // ---- Compatibility shim: webpack-dev-server v5 dropped several v4 options.
  // react-scripts 5.0.1 still uses them. Translate them so the dev server starts.
  ...
  if (legacyBefore || legacyAfter) {
    ...
    devServerConfig.setupMiddlewares = (middlewares, devServer) => {
      if (legacyBefore) {
        try { legacyBefore(devServer); } catch (e) { console.warn("legacy onBeforeSetupMiddleware error", e); }
      }
      const next = userSetup ? userSetup(middlewares, devServer) : middlewares;
      if (legacyAfter) {
        try { legacyAfter(devServer); } catch (e) { console.warn("legacy onAfterSetupMiddleware error", e); }
      }
      return next;
    };
  }
  // `https` option was removed in v5 \u2014 use `server: { type: 'https' | 'http' }`.

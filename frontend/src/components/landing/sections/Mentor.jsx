...
const MentorOrb = ({ initials }) => {
  ...
  return (
    ...
    >
      {/* Outer halo */}
      <div className="absolute -inset-6 rounded-full bg-[#ff5d2a]/30 blur-3xl" aria-hidden />
      {/* Orbit ring */}
      <motion.div
        animate={{ rotate: 360 }}
      ...
      >
        <span className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rounded-full bg-[#ff5d2a] shadow-[0_0_16px_3px_rgba(255,93,42,0.6)]" />
      </motion.div>
            ...
            <div className="relative h-full border-r border-b border-hair border-cream-10 bg-[rgba(12,20,40,0.3)] p-8 md:p-12">
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {MENTOR.lead.chips.map((c, i) => (
                  <motion.span
                    key={i}
                    whileHover={{ y: -2, borderColor: "rgba(255,93,42,0.45)", color: "#efe7d6" }}
          ...
          <div className="flex flex-col border-b border-hair border-cream-10 md:col-span-5">
            ...
            {MENTOR.team.map((m, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <motion.div
                  whileHover={{ x: 8, backgroundColor: "rgba(255,93,42,0.04)" }}
                  transition={{ type: "spring", stiffness: 220, damping: 22 }}
                  className="group flex items-center gap-4 border-t border-hair border-cream-10 px-8 py-6 md:px-10 cursor-pointer"
                >
                  <motion.div
                    whileHover={{ rotate: 360, scale: 1.08 }}
                    transition={{ duration: 0.9 }}
                    className="grid h-12 w-12 shrink-0 place-items-center rounded-full border-hair border-cream-15 bg-[rgba(255,93,42,0.06)] font-mono text-[12px] tracking-wider text-lava-soft"
                  >
                    {m.initials}
                  </motion.div>
